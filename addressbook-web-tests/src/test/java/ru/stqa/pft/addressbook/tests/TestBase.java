...
public class TestBase {
    ...
    if (Boolean.getBoolean("verifyUI")) {
      ...
      assertThat(uiContacts, equalTo(dbContacts.stream()
              .map((g) -> new ContactData().withId(g.getId()).withFirstName(g.getFirstName()).withLastName(g.getLastName())
                      .withAllPhones(mergePhones(g)).withAllEmails(mergeEmails(g)).withAddress(g.getAddress()))
              .collect(Collectors.toSet())));
    }
  }
  ...
  protected static String cleaned(String phone) {
